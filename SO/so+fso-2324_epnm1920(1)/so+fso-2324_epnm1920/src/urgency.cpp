...
void init_simulation(int np)
{
   ...
   hd = (HospitalData*)malloc(sizeof(HospitalData)); // mem_alloc is a malloc with NULL pointer verification
...
void* patient_main(void* agr) {
   int id = (*(int*) agr);
   patient_life(id);
   return 0;
}

void* nurse_main(void* agr) {
   int id = (*(int*) agr);
   int flag = 0;
   while (flag == 0) {
...
int main(int argc, char *argv[])
{
   ...
   pthread_t every_patients[npatients];
   pthread_t every_nurses[nnurses];
   pthread_t every_doctors[ndoctors];
   int id_patients[npatients];
   int id_nurses[nnurses];
   int id_doctors[ndoctors];
   for( int i = 0; i < npatients; i++) {
      id_patients[i] = i;
      thread_create(&every_patients[i], NULL, patient_main, (void*) &id_patients[i]);
   }
   for( int i = 0; i < nnurses; i++) {
      id_nurses[i] = i;
      thread_create(&every_nurses[i], NULL, nurse_main, (void*) &id_nurses[i]);
   }
   for( int i = 0; i < ndoctors; i++) {
      id_doctors[i] = i;
      thread_create(&every_doctors[i], NULL, doctor_main, (void*) &id_doctors[i]);
   }
   for (int i = 0; i<npatients;i++) {
      thread_join(every_patients[i], NULL);
   }
   for (int i = 0; i < nnurses;i++) {
   ...
   for (int i = 0; i<nnurses;i++) {
      thread_join(every_nurses[i], NULL);
   }
   for (int i = 0; i<ndoctors;i++) {
      thread_join(every_doctors[i], NULL);
   }
