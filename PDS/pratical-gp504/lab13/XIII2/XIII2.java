...
public class XIII2 {
	...
	private static void question2(PrintStream out) {
		...
		// Create and add a ShipOfSmallShips containing 3 PassengerShips
		ShipOfSmallShips shipOfSmallShips = new ShipOfSmallShips("C01", "Container Ship");
		shipOfSmallShips.add(new PassengerShip("B899", "Bora", 4));
		shipOfSmallShips.add(new PassengerShip("B878", "Riacho", 2));
		shipOfSmallShips.add(new PassengerShip("B785", "Turista", 8));
		av.add("C01", shipOfSmallShips);

		// Iterate over the ships and print their details
		Iterator<String> it = av.iterator();
		while (it.hasNext()) {
			out.println(it.next());
		}

		// Adding additional ships to match the expected output
