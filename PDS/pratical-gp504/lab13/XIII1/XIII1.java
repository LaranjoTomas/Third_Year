...
public class XIII1 {
	...
	private static void question2(PrintStream out) {
		...
		ProductsReader Reader = new Reader("./lab13/XIII1/products.txt");
		ToShare market = new ToShare(Reader);

		List<Product> cars = market.getProducts();
		...
		Client u2 = new Client("957", "Anne Marques");
		Client u3 = new Client("826", "Mary Monteiro");
		market.share("990000", u1);
		market.share("970006", u2);
		market.share("900010", u3);

		out.println("--- Shared Products :\n" + market.sharedProducts());
		market.giveBack("900010");
		market.giveBack("970006 ");
		out.println("--- Shared Products :\n" + market.sharedProducts());
	}
